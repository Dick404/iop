...
class mysql_installer():
    ...
    def install_mysql(self,ips):
        for ip in ips:
            ...
            os.system("ssh "+ip+" systemctl enable mariadb")
...
# ip arguments can't include proxy-ip and monitor-ip 
    def set_mysql(self,num,typeof,ip,lenth=None):
        if not os.path.exists("/tmp/iop/galera.cnf"):
            reader = open("./config/galera.cnf","r")
            configs = reader.readlines()
            reader.close()
        else :
	    reader = open("/tmp/iop/galera.cnf","r")
	    configs = reader.readlines()
	    reader.close()
        writer = open("/tmp/iop/galera.cnf","w")
        for config in configs:
	        ...
	        writer.write(data)
        writer.close()
        os.system("scp /tmp/iop/galera.cnf node"+str(num+2)+":/etc/my.cnf.d/")
	if typeof == "master":
	    ...
	    os.system("ssh node"+str(num+2)+" mysql_install_db")
	    os.system("ssh node"+str(num+2)+" systemctl stop firewalld.service;systemctl disable firewalld.service")
            os.system("ssh node"+str(num+2)+" \"systemctl start mariadb.service\"")
        return
    ...
    def operator(self,ips):
        ...
        os.system("rm -rf /tmp/iop/galera.cnf")                  # if not repeate excutation will produce error
        self.init_mysql()
        return 
